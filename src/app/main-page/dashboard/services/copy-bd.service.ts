...
export class CopyBDService {
    ...
    constructor(private operationTaskService: OperationTaskService, private http: HttpClient
    ) {
        this.getTasks()
    ...
    public getTasks() {
        if (!!this.allTaskSubject$.value) {
            this.operationTaskService.getAllTask().pipe(
                take(1)).subscribe(
                (tasks: ITask[]) => {
